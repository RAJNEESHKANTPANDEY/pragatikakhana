...
function loadData() {
  if (!fs.existsSync(DATA_FILE)) {
    const defaults = {
      pantry: ["Rice","Wheat flour","Moong dal","Toor dal","Chana dal","Onion","Tomato","Garlic","Ginger","Green chilli","Turmeric","Cumin seeds","Mustard seeds","Coriander powder","Garam masala","Ghee","Oil","Salt","Milk","Eggs","Potatoes","Spinach","Lemon juice","Coriander leaves","Curry leaves"],
      customDishes: [],
...
app.post('/api/pantry', (req, res) => {
  ...
  toAdd.forEach(i => { const t = i.trim(); if (t && !d.pantry.map(p => p.toLowerCase()).includes(t.toLowerCase())) d.pantry.push(t); });
  saveData(d);
...
app.post('/api/dishes', (req, res) => {
  ...
  if (!name || !category || !description) return res.status(400).json({ success: false, error: 'name, category, description are required' });
  const d = loadData();
  const dish = { id: uid(), name: name.trim(), emoji: emoji||'🍽️', origin: (origin||'My Kitchen').trim(), category, type: type||'veg', time: parseInt(time)||30, easy: Boolean(easy), description: description.trim(), ingredients: Array.isArray(ingredients)?ingredients:[], recipe: (recipe||'').trim(), tips: (tips||'').trim(), custom: true, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
...
app.delete('/api/dishes/:id', (req, res) => {
  ...
  if (d.customDishes.length === before) return res.status(404).json({ success: false, error: 'Dish not found' });
  saveData(d);
...
app.listen(PORT, () => { console.log(`✅  Roz Ka Khana → http://localhost:${PORT}`); loadData(); });
